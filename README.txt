...
2. Once the code is compiled, find the atlantis exectutable file (atlantismain.exe) and put it into the same folder as your input files.
...
6. If the NetCDF libraries are installed properly, then Atlantis should run. Otherwise you might need to find missing .dll files (usually netcdf.dll and MSVCR71.dll) and put them in the same folder 
as atlantismain.exe file. If some .dll files are missing, Atlantis will usually give an error message with the missing .dll file name. 
...
####################
For instruction on how to install and run Atlatnis on Linux and MacOS machine please see https://github.com/Atlantis-Ecosystem-Model/Atlantis_example_and_instructions
